...
internal object CredentialIssuerMetadataSerializer : KSerializer<CredentialIssuerMetadata> {
    ...
    override fun deserialize(decoder: Decoder): CredentialIssuerMetadata {
        ...
        // Skip configs that fail validation (e.g. foreign issuers advertising ldp_vc with
        // JOSE algs like ES256, or legacy format quirks). Keep usable entries so metadata
        // resolve does not hard-fail for the whole issuer.
        val credentialConfigurationsSupported =
            credentialConfigurationsElement.jsonObject.mapNotNull { (id, element) ->
                try {
                    id to lenientJson.decodeFromJsonElement(CredentialConfiguration.serializer(), element)
                } catch (_: Exception) {
                    null
                }
            }.toMap()
        if (credentialConfigurationsSupported.isEmpty()) {
            throw SerializationException(
                "credential_configurations_supported contained no usable entries " +
                    "(all failed validation or deserialization)",
            )
        }
