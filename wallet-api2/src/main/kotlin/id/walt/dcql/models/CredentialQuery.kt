...
data class CredentialQuery(
    ...
    init {
        ...
        if (metaFormat != null) {
            // Legacy sandboxes (eduWallet) often pair vc+sd-jwt with W3C-style type_values.
            val legacySdJwtTypeValues = format == CredentialFormat.DC_SD_JWT && meta is id.walt.dcql.models.meta.JwtVcJsonMeta
            require(metaFormat == format || legacySdJwtTypeValues) {
                "Invalid query \"meta\" for this format. Credential format is set to \"$format\", but query meta is for \"$metaFormat\"."
            }
        }
    }
}
