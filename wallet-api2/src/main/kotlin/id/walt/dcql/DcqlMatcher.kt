...
object DcqlMatcher {
    ...
    private fun claimExistsAndMatchesValue(
        ...
        if (isCredentialPotentiallySD && credential.disclosures != null) {
            ...
            if (matchingDisclosure != null) {
                val requiredValues = claimQuery.values
                if (!requiredValues.isNullOrEmpty()) {
                    val disclosureValueJson = matchingDisclosure.value
                    val matchesValue = requiredValues.any { queryValue -> queryValue == disclosureValueJson }
                    if (!matchesValue) {
                        log.trace { "SD Disclosure '${targetClaimName}' value '${disclosureValueJson}' does not match required values $requiredValues in ${credential.id}" }
        ...
        val requiredValues = claimQuery.values
        if (!requiredValues.isNullOrEmpty()) {
            val matchesValue = requiredValues.any { queryValue -> queryValue == claimJsonElement }
            if (!matchesValue) {
                log.trace { "claimExistsAndMatchesValue: Claim path ${claimQuery.path} value '$claimJsonElement' does not match required values $requiredValues in ${credential.id} " }
    ...
    private fun matchesMeta(
        ...
        val formatMatchesQueryType = when (expectedFormat) {
            ...
            // Accept JwtVcJsonMeta for SD-JWT: some sandboxes send type_values with vc+sd-jwt
            CredentialFormat.DC_SD_JWT -> metaQuery is SdJwtVcMeta || metaQuery is JwtVcJsonMeta
        ...
        return when (metaQuery) {
            is JwtVcJsonMeta -> {
                val isSdJwt = !isNotAllowedMetaFormat(CredentialFormat.DC_SD_JWT)
                if (isNotAllowedMetaFormat(CredentialFormat.JWT_VC_JSON, CredentialFormat.LDP_VC, CredentialFormat.DC_SD_JWT)) {
                    log.warn { "W3cCredentialMeta applied to non-W3C/SD-JWT format ${credential.format} for ${credential.id}" }
                    return false
                }
                if (isSdJwt) {
                    // Legacy type_values against SD-JWT: match vct and/or W3C-style type array
                    val vctClaim = credential.data["vct"]?.jsonPrimitive?.contentOrNull
                    val credTypesElement = credential.data["type"] ?: credential.data["vc"]?.jsonObject?.get("type")
                    val credTypes = (credTypesElement as? JsonArray)
                        ?.mapNotNull { it.jsonPrimitive.contentOrNull }
                        .orEmpty()
                    return metaQuery.typeValues.any { requiredTypeSet ->
                        requiredTypeSet.all { requiredType ->
                            requiredType == vctClaim || credTypes.contains(requiredType)
                        }
                    }
                }
    ...
    private fun claimExistsAndMatches(credential: DcqlCredential, claimQuery: ClaimsQuery): Boolean {
        ...
        if (!requiredValues.isNullOrEmpty()) {
            ...
            val matchesValue = requiredValues.any { queryValue ->
                // Simple comparison for primitives. Might need type coercion logic.
                queryValue == claimValue
            }
            if (!matchesValue) {
                log.trace { "claimExistsAndMatches: Claim path ${claimQuery.path} value '$claimValue' does not match required values $requiredValues in credential ${credential.id}" }
                return false
            }
        }
