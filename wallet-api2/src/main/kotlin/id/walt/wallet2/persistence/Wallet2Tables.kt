...
object Wallet2Tables {
    ...
    object Wallets : Table("wallet2_wallets") {
        ...
        /** Friendly UI label; null → clients fall back to a short wallet id. */
    ...
    /**
     * Auditable wallet event log (receive/present/keys/DIDs).
     * No FK to wallets so events remain even if a wallet row is deleted later.
     */
    object Events : Table("wallet2_events") {
        val id = integer("id").autoIncrement()
        val walletId = varchar("wallet_id", 128).index()
        val event = varchar("event", 128)
        val action = varchar("action", 128)
        val timestamp = timestamp("timestamp")
        val tenant = varchar("tenant", 128)
        val originator = varchar("originator", 256).nullable()
        val credentialId = varchar("credential_id", 256).nullable()
        val data = text("data")
