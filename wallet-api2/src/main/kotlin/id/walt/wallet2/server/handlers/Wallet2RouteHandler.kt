...
import id.walt.wallet2.server.events.eventData
...
object Wallet2RouteHandler {
    ...
    internal fun Route.registerWalletManagementRoutes(
        ...
        route("/{walletId}") {
            ...
            route("/keys", { tags = listOf("Key Management") }) {
                ...
                delete("", {
                    summary = "Delete a key by query parameter"
                    description = "Preferred: `DELETE .../keys?keyId={id}` — avoids path-encoding issues with `secdsa:…` ids."
                    request {
                        pathParameter<String>("walletId")
                        queryParameter<String>("keyId") { required = true }
                    }
                    ...
                    val keyId = call.request.queryParameters["keyId"]
                        ?: return@delete call.respond(HttpStatusCode.BadRequest, "Missing query parameter 'keyId'")
                ...
                post("/generate", Wallet2OpenApiDocs.generateKey()) {
                    ...
                    val backend = json["backend"]?.jsonPrimitive?.contentOrNull ?: "secdsa"
                    // Chapter F Phase 2: this demo holder only generates SECDSA SoftHSM keys.
                    if (backend != "secdsa") {
                        return@post call.respond(
                            HttpStatusCode.BadRequest,
                            "This wallet only supports backend=secdsa (got '$backend')",
                        )
                    }
                    ...
                    Wallet2EventLog.append(
                        ...
                        event = "Key",
                        action = "Create",
                        data = eventData("keyId" to keyId, "keyType" to keyTypeName, "backend" to backend),
                    )
                    call.respond(HttpStatusCode.Created, WalletKeyInfo(keyId = keyId, keyType = keyTypeName))
                }
                ...
                post("/import", {
                    ...
                    Wallet2EventLog.append(
                        ...
                        event = "Key",
                        action = "Import",
                        data = eventData("keyId" to keyId, "keyType" to key.keyType.name),
                    )
                    call.respond(HttpStatusCode.Created, WalletKeyInfo(keyId = keyId, keyType = key.keyType.name))
                }

                post("/verify", {
                    summary = "Verify a JWS with a public JWK"
                    description = "Query parameter `JWK` holds the public JWK JSON; body is the compact JWS string."
                    request {
                        pathParameter<String>("walletId")
                        queryParameter<String>("JWK") { required = true }
                        body<String> { required = true }
                    }
                    response { HttpStatusCode.OK to { body<Boolean>() } }
                }) {
                    val jwk = call.request.queryParameters["JWK"]
                        ?: return@post call.respond(HttpStatusCode.BadRequest, "Missing JWK query parameter")
                    val signature = call.receiveText()
                    runCatching {
                        val key = JWKKey.importJWK(jwk).getOrThrow()
                        key.verifyJws(signature).isSuccess
                    }.onSuccess {
                        call.respond(it)
                    }.onFailure {
                        call.respond(HttpStatusCode.BadRequest, it.message ?: "verify failed")
                    }
                }
                ...
                route("/{keyId}") {
                    ...
                    get("/load", {
                        summary = "Load key as JWK object"
                        description =
                            "Returns exportJWKObject(). Software keys may include private `d`; " +
                                "SECDSA SoftHSM keys export public material only."
                        request { pathParameter<String>("walletId"); pathParameter<String>("keyId") }
                        ...
                        val key = wallet.findKey(keyId)
                            ?: return@get call.respond(HttpStatusCode.NotFound, "Key '$keyId' not found")
                        call.respond(key.exportJWKObject())
                    }

                    get("/meta", {
                        summary = "Load cryptographic key meta"
                        request { pathParameter<String>("walletId"); pathParameter<String>("keyId") }
                        response { HttpStatusCode.OK to { body<JsonObject>() } }
                    }) {
                        val wallet = call.resolveOrRespond(resolver, getAccountId) ?: return@get
                        ...
                        val key = wallet.findKey(keyId)
                            ?: return@get call.respond(HttpStatusCode.NotFound, "Key '$keyId' not found")
                        call.respond(Json.encodeToJsonElement(key.getMeta()).jsonObject)
                    }

                    get("/export", {
                        summary = "Export key as JWK or PEM string"
                        request {
                            pathParameter<String>("walletId")
                            pathParameter<String>("keyId")
                            queryParameter<String>("format") { description = "JWK (default) or PEM" }
                            queryParameter<Boolean>("loadPrivateKey") {
                                description = "Include private material when the backend supports it"
                            }
                        }
                        ...
                        val private = call.request.queryParameters["loadPrivateKey"]?.toBoolean() == true
                        val key = wallet.findKey(keyId)
                            ?: return@get call.respond(HttpStatusCode.NotFound, "Key '$keyId' not found")
                        runCatching {
                            when (format.lowercase()) {
                                "jwk" -> if (private) key.exportJWK() else key.getPublicKey().exportJWK()
                                "pem" -> if (private) key.exportPEM() else key.getPublicKey().exportPEM()
                                else -> throw IllegalArgumentException("Unknown format: $format")
                            }
                        }.onSuccess {
                            call.respondText(it, ContentType.Text.Plain)
                        }.onFailure {
                            call.respond(HttpStatusCode.BadRequest, it.message ?: "export failed")
                        }
                    }

                    post("/sign", {
                        summary = "Sign a payload as compact JWS"
                        description =
                            "Body is arbitrary JSON (serialized as JWT payload) or a Flattened JWS JSON " +
                                "object with only `protected` + `payload`. SECDSA keys require an unlocked PIN session."
                        request {
                            pathParameter<String>("walletId")
                            pathParameter<String>("keyId")
                            body<JsonElement> { required = true }
                        }
                        response { HttpStatusCode.OK to { body<String>() } }
                    }) {
                        val wallet = call.resolveOrRespond(resolver, getAccountId) ?: return@post
                        val keyId = call.parameters["keyId"]!!
                        val key = wallet.findKey(keyId)
                            ?: return@post call.respond(HttpStatusCode.NotFound, "Key '$keyId' not found")
                        val data = call.receive<JsonElement>()
                        runCatching {
                            signWithWalletKey(keyId, key, data)
                        }.onSuccess {
                            Wallet2EventLog.append(
                                walletId = wallet.id,
                                event = "Key",
                            ...
                            log.warn(it) { "sign failed for key $keyId" }
                            call.respond(HttpStatusCode.BadRequest, it.message ?: "sign failed")
                        }
                    }
            ...
            route("/dids", { tags = listOf("DID Management") }) {
                ...
                delete("", {
                    summary = "Delete a DID by query parameter"
                    description = "Preferred delete API: `DELETE .../dids?did={did}`. " +
                        "Avoids path-encoding issues with `did:jwk:…` / `did:key:…` in browsers."
                    request {
                        pathParameter<String>("walletId")
                        queryParameter<String>("did") { required = true }
                    }
                    response { HttpStatusCode.NoContent to {}; HttpStatusCode.NotFound to {} }
                }) {
                    val wallet = call.resolveOrRespond(resolver, getAccountId) ?: return@delete
                ...
                post("/create", Wallet2OpenApiDocs.createDid()) {
                    ...
                    val key = (req.keyId?.let { wallet.findKey(it) }
                        ?: wallet.defaultKey()
                        ?: wallet.listAllKeys().firstOrNull()?.let { wallet.findKey(it.keyId) })
                        ?: return@post call.respond(
                            HttpStatusCode.BadRequest,
                    ...
                    Wallet2EventLog.append(
                        ...
                        event = "DID",
                        action = "Create",
                        data = eventData("did" to entry.did, "method" to req.method),
                    )
                ...
                post("/import", {
                    ...
                    Wallet2EventLog.append(
                        ...
                        event = "DID",
                        action = "Import",
                        data = eventData("did" to entry.did),
                    )
                    call.respond(HttpStatusCode.Created, entry)
                }
                ...
                route("/{did}") {
                    ...
                    delete("", {
                        ...
                        description = "Pass the DID as path parameter (URL-encoded) or as query parameter `did` " +
                            "(preferred — avoids client URL-normalization issues with `did:jwk:…`)."
                        request {
                            pathParameter<String>("walletId")
                            pathParameter<String>("did")
                            queryParameter<String>("did") { description = "Alternative to path parameter" }
                        }
                        response { HttpStatusCode.NoContent to {}; HttpStatusCode.NotFound to {} }
                    }) {
                        val wallet = call.resolveOrRespond(resolver, getAccountId) ?: return@delete
            ...
            route("/credentials") {
                ...
                post("/import", {
                    ...
                    val stored = WalletCredentialHandler.importCredential(wallet, req)
                    Wallet2EventLog.append(
                        walletId = wallet.id,
                        event = "Credential",
                        action = "Import",
                        credentialId = stored.id,
                        data = eventData("credentialId" to stored.id, "format" to stored.credential.format),
                    )
                    ...
                    delete("", {
                        ...
                        if (removed) {
                            Wallet2EventLog.append(
                                ...
                                data = eventData("credentialId" to credentialId),
                            )
                            call.respond(HttpStatusCode.NoContent)
                    ...
                    post("", {
                        ...
                        try {
                            ...
                            Wallet2EventLog.append(
                                ...
                                action = "Receive",
                                data = eventData(
                                    "credentialIds" to result.credentialIds.joinToString(","),
                                    "count" to result.credentialIds.size.toString(),
                                ),
                            )
                            call.respond(result)
                        } catch (e: Exception) {
            ...
            // Classic-compatible VCT metadata helper (used by web wallet credential titles)
            route("/exchange", { tags = listOf("Utilities") }) {
                get("/resolveVctUrl", {
                    summary = "Resolve SD-JWT VC type metadata from a VCT URL"
                    request {
                        pathParameter<String>("walletId")
                        queryParameter<String>("vct") { required = true }
                    }
                    response { HttpStatusCode.OK to { body<SdJwtVcTypeMetadataDraft04>() } }
                }) {
                    // walletId is accepted for path compatibility with classic wallet-api
                    val vct = call.request.queryParameters["vct"]
                        ?: return@get call.respond(HttpStatusCode.BadRequest, "Missing vct query parameter")
                    runCatching { resolveVctMetadata(vct) }
                        .onSuccess { call.respond(it) }
                        .onFailure {
                            call.respond(HttpStatusCode.BadRequest, it.message ?: "VCT resolve failed")
                        }
                }
            }
            ...
            get("/eventlog", {
                ...
                summary = "Retrieve auditable event log for this wallet"
                request {
                    pathParameter<String>("walletId")
                    queryParameter<Int>("limit") { description = "Page size" }
                    queryParameter<String>("startingAfter") { description = "Offset index as string" }
                }
                response { HttpStatusCode.OK to { body<Wallet2EventLogPage>() } }
            }) {
                val wallet = call.resolveOrRespond(resolver, getAccountId) ?: return@get
                val limit = call.request.queryParameters["limit"]?.toIntOrNull() ?: 10
                val startingAfter = call.request.queryParameters["startingAfter"]
                call.respond(Wallet2EventLog.list(wallet.id, limit = limit, startingAfter = startingAfter))
            }
        }
    }

    // -------------------------------------------------------------------------
    // Top-level utilities (classic wallet-api compatibility)
    // -------------------------------------------------------------------------

    internal fun Route.registerUtilityRoutes() {
        get("/transaction-data-profiles", {
            ...
            val profiles = runCatching {
                ConfigManager.getConfig<TransactionDataProfilesConfig>().transactionDataProfiles
            }.getOrElse { emptyList() }
        ...
        route("/util", { tags = listOf("Utilities") }) {
            post("/parseMDoc", {
                summary = "Parse MDOC document (CBOR hex) to JSON"
                request { body<String> { required = true } }
                response { HttpStatusCode.OK to { description = "Parsed MDoc JSON" } }
            }) {
                val mdoc = call.receiveText()
                runCatching {
                    DataElement.fromCBORHex<DataElement>(mdoc).toJsonElement()
                }.onSuccess {
                    call.respond(it)
                }.onFailure {
                    call.respond(HttpStatusCode.BadRequest, it.message ?: "parseMDoc failed")
                }
            }
        }
    }
    ...
    private suspend fun signWithWalletKey(
        keyId: String,
        ...
        return if (jwsObj != null) {
            val headerB64 = jwsObj["protected"]?.jsonPrimitive?.content
                ?: throw IllegalArgumentException("Missing protected header")
            val payloadB64 = jwsObj["payload"]?.jsonPrimitive?.content
                ?: throw IllegalArgumentException("Missing payload")
            var headers = Json.parseToJsonElement(
            ...
            when {
                headers.containsKey("kid") -> {
                    ...
                    if (kid != keyId) throw IllegalArgumentException("JWT headers.kid must match keyId: $keyId")
                }
                headers.containsKey("jwk") -> {
                    val jwkKid = headers.getValue("jwk").jsonObject["kid"]?.jsonPrimitive?.content
                    if (jwkKid != null && jwkKid != keyId) {
                        throw IllegalArgumentException("JWT headers.jwk.kid must match keyId: $keyId")
                    }
                }
                else -> {
                    headers = JsonObject(headers + ("kid" to JsonPrimitive(keyId)))
                }
            }
            val payloadBytes = Base64.UrlSafe.withPadding(Base64.PaddingOption.ABSENT_OPTIONAL)
                .decode(payloadB64)
            key.signJws(payloadBytes, headers)
        } else {
            val headers = buildJsonObject {
                put("typ", "JWT")
                put("kid", keyId)
            }
            key.signJws(data.toString().encodeToByteArray(), headers)
        }
    }

    private val vctHttpClient by lazy {
        HttpClient(CIO) {
    ...
    private suspend fun resolveVctMetadata(vct: String): SdJwtVcTypeMetadataDraft04 {
        val url = Url(vct)
        val authority = url.protocolWithAuthority
        val path = vct.substringAfter(authority)
        val response = vctHttpClient.get("$authority/.well-known/vct$path")
        require(response.status.isSuccess()) { "VCT URL returns error: ${response.status}" }
        return SdJwtVcTypeMetadataDraft04.fromJSON(response.body<JsonObject>())
    }
}
