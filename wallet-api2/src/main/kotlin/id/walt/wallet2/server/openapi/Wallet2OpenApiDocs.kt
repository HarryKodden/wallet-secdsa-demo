...
object Wallet2OpenApiDocs {
    ...
    fun generateKey(): RouteConfig.() -> Unit = {
        ...
        description = """
            ...
            Built-in `backend` values: `jwk` (default), `tse`, `azure-rest-api`, `oci-rest-api`,
            `aws-rest-api`. Additional backends registered on `KeyManager` at process startup
            (for example educational `secdsa`) are accepted via the legacy generation payload:
            `{"backend":"secdsa","keyType":"secp256r1","config":{...}}`.
