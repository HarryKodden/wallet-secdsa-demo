...
interface WalletResolver {
    ...
    /**
     * Sets or clears the friendly [displayName] for an existing wallet.
     * Empty / blank clears the name (clients show a short-id fallback).
     */
    suspend fun setWalletDisplayName(walletId: String, displayName: String?) {
        val normalized = displayName?.trim()?.take(128)?.takeIf { it.isNotEmpty() }
        // Always persist via descriptor (ExposedWalletStore). Also update a live
        // in-memory wallet when present so the current process sees the new name.
        walletStore.loadWallet(walletId)?.let {
            walletStore.saveWallet(it.copy(displayName = normalized))
        }
        val descriptor = walletStore.loadDescriptor(walletId)
            ?: error("Wallet '$walletId' not found when setting displayName")
        walletStore.saveDescriptor(descriptor.copy(displayName = normalized))
    }
}
