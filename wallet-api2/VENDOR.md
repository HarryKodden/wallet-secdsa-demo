...
| `WalletPresentationHandler`, `PreviewSessionStore`, `WalletIssuanceHandler`, `JwtProofBuilder` | Fork overlay: holder key/DID from credential `cnf` (not wallet default), `clientId` on proof JWT `iss`, preview session store |
