...
| `WalletPresentationHandler`, `PreviewSessionStore`, `WalletIssuanceHandler`, `JwtProofBuilder` | Fork overlay: holder key/DID from credential `cnf` (not wallet default), `clientId` on proof JWT `iss`, preview session store; remaps W3C+SD-JWT `jwt_vc_json` → `vc+sd-jwt` for DCQL (stock sealed W3C classes cannot be overlaid) |
