...
async function importKey() {
    const trimmed = keyText.value.trim();
    let key: Record<string, unknown>;
    try {
        key = JSON.parse(tryInjectAliasIntoJwk(trimmed, alias.value));
    } catch {
        window.alert("wallet-api2 key import expects a JWK JSON object (PEM is not supported here).");
        return;
    }
