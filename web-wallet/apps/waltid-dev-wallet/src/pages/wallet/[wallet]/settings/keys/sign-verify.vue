  ...
  <CenterMain>
    <div>
      ...
      <div class="mb-8">
        ...
        >
          <option v-for="key in keys" :key="keyIdOf(key)" :value="keyIdOf(key)">
            {{ keyIdOf(key) }} ({{ key.keyType || key.algorithm || "key" }})
          </option>
        </select>
...
async function signMessage() {
  try {
    ...
    const result = await $fetch(
        `/wallet-api/wallet/${currentWallet.value}/keys/${encodeURIComponent(selectedKey.value)}/sign`,
        {
          method: "POST",
          body: messageObj,
        }
    ...
    signature.value = typeof result === "string" ? result : String(result);
  } catch (error) {
    if (error instanceof SyntaxError) {
      alert("Invalid JSON format. Please check your message.");
    } else {
      console.error("Signing error:", error);
      alert("An error occurred while signing the message (SECDSA needs PIN unlock first).");
    }
  }
}
