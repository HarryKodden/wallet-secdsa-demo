    ...
    <CenterMain>
        ...
        <div class="mt-6 border p-4 rounded-2xl">
            ...
            <div class="mt-4 flex items-center justify-end gap-x-6">
                <p v-if="actionError" class="mr-auto rounded-xl bg-red-50 p-3 text-sm text-red-800">
                    {{ actionError }}
                </p>
                <button
...
const rawKeyParam = route.params.keyId;
const keyId = decodeURIComponent(
  Array.isArray(rawKeyParam) ? rawKeyParam[0] : String(rawKeyParam ?? ""),
);
...
async function loadData() {
    try {
        jwk.value = await $fetch(
            `/wallet-api/wallet/${currentWallet.value}/keys/${encodeURIComponent(keyId)}/load`,
        );
    } catch (e) {
        jwk.value = null;
    }
    try {
        meta.value = await $fetch(
            `/wallet-api/wallet/${currentWallet.value}/keys/${encodeURIComponent(keyId)}/meta`,
        );
    } catch (e) {
        meta.value = null;
    }
...
function exportKey() {
  navigateTo(
    `export/${encodeURIComponent(keyId)}?format=${format.value}&loadPrivateKey=${enableLoadPrivateKey.value}`,
  );
}
...
async function deleteKey() {
  ...
  actionError.value = "";
  try {
    await $fetch(`/wallet-api/wallet/${currentWallet.value}/keys`, {
      method: "DELETE",
      query: {keyId},
    });
    await navigateTo(`/wallet/${currentWallet.value}/settings/keys`);
  } catch (e: any) {
