    ...
    <CenterMain>
      ...
      <div class="flex col-2">
        <div v-if="!pendingDids" class="relative w-full">
          ...
          <div
            v-if="!pendingDids && (!dids || dids.length === 0)"
            class="rounded-md bg-amber-50 p-3 mb-3 text-sm text-amber-900"
          >
            This wallet has no DID yet, so <strong>Accept</strong> stays disabled.
            Create a SECDSA key and DID first:
            <NuxtLink
              class="font-medium underline"
              :to="`/wallet/${currentWallet}/settings/keys`"
            >Keys</NuxtLink>
            → generate <em>SECDSA SoftHSM</em>, then
            <NuxtLink
              class="font-medium underline"
              :to="`/wallet/${currentWallet}/settings/dids`"
            >DIDs</NuxtLink>
            → create <em>did:jwk</em> from that key. Then return here and Accept.
          </div>
